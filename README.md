...
## Outreach from the user's Gmail + read tracking

* A user may connect Gmail (scope `gmail.send` only - it cannot read the mailbox). Outreach
  then goes out From her own address and lands in her Sent folder. Only the refresh token
  is stored, Fernet-encrypted with `GMAIL_TOKEN_KEY` (32+ chars; generated by Render).
  Disconnecting revokes it at Google. A revoked/expired grant disconnects automatically.
* Every outreach email (Gmail or site SMTP) carries a 1x1 image with a random per-message
  token (`/api/o/<token>.gif`). Loads within 60 s of sending are ignored (the sender's own
  preview, link scanners). The personal area lists sent emails with ✓ sent / ✓✓ opened.
  It is a signal, not proof: blocked images never report; Apple Mail preloads images.
* Setup (Gmail API, redirect URI, scope, Google verification limits): DEPLOY.md §5א.
